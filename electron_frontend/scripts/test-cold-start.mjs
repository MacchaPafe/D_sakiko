...
assert.match(stage, /let disposed = false/, 'Stage must track async disposal')
assert.match(stage, /disposed = true/, 'Stage must mark itself disposed on unmount')
assert.match(stage, /if \(disposed\) \{[\s\S]*?destroy\?\.\(/, 'Stage must destroy a model loaded after unmount')
assert.match(stage, /if \(disposed\) return/, 'Stage must stop setup after unmount')
