...
**角色**:
AI 在对话中扮演的对象，拥有稳定身份、显示名称和角色描述。Live2D、语音与头像是角色可选能力，不构成角色本身。
_Avoid_: 系统角色、Live2D 角色、语音角色

**角色 ID**:
角色在本地持续使用的稳定身份，由其角色资源文件夹名称表示；修改显示名称不会改变角色 ID。
_Avoid_: 角色显示名称、角色 UUID、Live2D 模型名称

**角色描述**:
由用户明确编写并确认、用于界定角色设定和扮演方式的非空提示内容，是角色参与对话的必要信息。示例或自动生成的占位文本不能自行成为正式角色描述。
_Avoid_: 自动角色模板、空 system prompt、角色名称

**对话身份**:
用户在对话中采用的身份，可以不包含任何人设信息，也可以拥有自定义描述或引用一个既有角色。它不是 AI 当前扮演的角色。
_Avoid_: 用户角色、角色

**待启用角色**:
已经由角色管理面板完整写入本地资源、但尚未通过程序重启进入运行时角色目录的角色。它可以继续在创建面板中查看和编辑，但不能参与当前运行中的对话或演出。
_Avoid_: 不完整角色目录、已加载角色、热刷新角色

**不完整角色目录**:
缺少非空角色名称或非空角色描述、因而不能表示一个角色的本地目录。它不出现在角色列表中，也不能仅凭拥有 Live2D 或语音资源成为角色。
_Avoid_: 无模型角色、纯文字角色、角色

**角色展示能力**:
角色通过可加载的 Live2D 模型参与视觉演出的可选能力。没有该能力不影响角色参与文字对话。
_Avoid_: 角色可用性、角色身份

**无模型展示**:
角色没有配置 Live2D 模型，或已配置模型加载失败时采用的视觉演出状态；窗口仍保留背景、角色名、字幕与播放节奏，但不绘制角色模型。模型加载失败必须记录错误日志，不能被解释为正常的模型未配置。
_Avoid_: 关闭 Live2D、空角色、静默加载失败

**角色**:
能够参与 AI 对话的身份，由稳定角色 ID、显示名称和非空角色描述构成；Live2D、语音和头像都是可选能力。
_Avoid_: 系统角色、Live2D 角色

**角色 ID**:
角色资源文件夹的名称，是跨配置和资源引用保持稳定的角色身份；显示名称变化不应改变角色 ID。
_Avoid_: 角色名称、额外 UUID、character_id

**角色描述**:
由用户写入 `character_description.txt` 的非空角色设定，用作角色对话的 system prompt。
_Avoid_: 自动生成 prompt、模型描述

**对话身份**:
用户在对话中选择扮演的身份，与 AI 侧的角色是两个不同概念。
_Avoid_: 用户角色、角色

**待启用角色**:
已经完整写入磁盘、但要到程序重启后才进入全局角色目录的角色；角色面板可以在当前进程中单独展示和编辑它。
_Avoid_: 已加载角色、热更新角色

**不完整角色目录**:
缺少非空显示名称或非空角色描述的角色资源目录，不构成可参与对话的角色。
_Avoid_: 无模型角色、无语音角色

**角色展示能力**:
角色拥有并可加载 Live2D 模型时提供的可选视觉表现能力；它不是角色参与对话的必要条件。
_Avoid_: 角色身份、对话能力

**无模型展示**:
角色没有可绘制 Live2D 模型时仍保留背景、角色名和字幕的展示形态。
_Avoid_: 渲染失败、关闭 Live2D 窗口
...
**Live2D 模型导入**:
将项目外部的 Live2D 模型复制成角色自有的自包含模型，并在副本上执行项目规范化；角色没有默认模型时，首次导入进入默认模型目录，已有默认模型时进入可选模型目录。它不等同于切换已有模型，也不应直接改写用户原始模型目录。
将项目外部的 Live2D 模型自包含复制到角色资源目录，并在复制后的模型上执行项目规范化；角色没有默认模型时，首次导入形成默认模型，之后的导入形成额外模型。它不等同于切换已有模型，也不应直接改写用户原始模型目录。
_Avoid_: 选择模型、就地规范化、循环导入

**默认 Live2D 模型**:
角色在对话没有显式选择其他模型时使用的可选展示模型；角色可以没有默认模型。
_Avoid_: 必需模型、对话级模型

**Live2D 模型目标**:
一次角色展示切换明确要求尝试加载的模型路径；空目标表示正常未配置，非空目标即使失效也必须按原路径尝试。
_Avoid_: 自动回退模型、已加载模型

**Live2D 加载结果**:
渲染进程对非空模型目标的一次加载结果；失败只产生运行时日志和无模型展示，不改写持久模型目标。
_Avoid_: 角色配置状态、模型目标

**空模型槽位**:
小剧场中保留角色名和对话位置、但没有可绘制 Live2D 模型的槽位；它不应被解释为版本不一致。
_Avoid_: 隐藏模型、无效角色槽位
...
**角色乐队**:
项目当前支持角色所属的九支主要乐队，每名受支持角色只归入其中一支。它不泛指团体或组合，也不包含 CRYCHIC 等历史乐队。
_Avoid_: 角色团体、组合、历史乐队
...
**来源重新确认**:
审核产物的直接来源摘要过期后，审核者基于版本化消费投影差异明确接受当前来源、更新 `direct_sources` 并保留既有审核结果的开发侧操作。它不调用 LLM，不等同于重新生成，也不能覆盖缺失来源、范围错误或悬空引用等结构问题。
_Avoid_: allow-stale、手工修改 SHA、重新生成、发布绕过

**消费投影**:
某类 Stage 3 下游真正用于规范化、聚合或去重的直接来源内容之确定性表示，用于区分整文件变化是否影响该下游。投影具有类型和版本；本地基线不是正式世界书或发布输入。
_Avoid_: 整文件 SHA-256、审核基础摘要、Embedding
...
**世界书包依赖**:
一个世界书包声明其运行时内容还需要哪些其他世界书包的关系；依赖具有传递性，根包使用时递归引入完整无环依赖闭包，任一必需包不可用都会使根包检索不可用。参与同一对话检索的依赖包必须与根包共享剧情时间轴；不同时间轴的剧情组合需要先定义统一时间轴和明确的组合根包。
...
**我的内容**:
世界书查看器中对世界书 Override、孤立 Override 和世界书用户扩展的用户侧统称，用来帮助用户找到自己写过的内容。它不包含只有隐藏状态而没有 Override 的官方条目，也不是新的持久化内容类型。
_Avoid_: 我修改的、我添加的、用户状态层、已隐藏官方条目
...
**包范围条目**:
`scope_type` 为 `PACKAGE` 的 Lore 条目；在所属世界书包被当前作品直接选择或通过依赖引入时均可使用。它仍受时间轴、剧情分支、有效时间和可空学年约束，也不自动适用于未依赖该包的其他作品，即使这些作品属于同一品牌。
_Avoid_: 数据库全局条目、全品牌通用条目、GLOBAL、无条件适用

**作品范围条目**:
`scope_type` 为 `SERIES` 的 Lore 条目；只有当前对话根世界书包的 `series_id` 位于条目的 `series_ids` 中时才可使用。它即使存放在被后续作品依赖的包中，也不会仅因包依赖而自动传递给后续作品。
_Avoid_: 包范围条目、依赖继承条目、动画播出季度
...
**世界书对话上下文**:
一段对话在某一轮检索中采用的世界书剧情位置，包括根世界书包、从角色知识映射解析出的角色知识视角、剧情时间轴、剧情进度所确定的当前时间点、可空剧情学年和剧情分支。用户调整剧情位置或角色知识映射只影响后续检索，不会改写历史消息。
_Avoid_: 角色固定时间点、全局 RAG 设置、世界书索引状态

**世界书回合快照**:
一条真实用户发言在生成回复时冻结的世界书对话上下文；重新生成该回合或从该回合分叉时沿用这份上下文，之后的设置变化不会追溯修改它。旧消息没有快照时使用当时可用的当前设置，不声称能够恢复原剧情位置。
_Avoid_: 当前世界书设置、消息索引、工具调用记录

**世界书诊断记录**:
一次世界书对话轮次中检索候选、过滤与排序决定、实际直接注入、静默工具轨迹、模型原始回答、用户可见回答及最终状态的单一结构化记录；内存详情、短期滚动日志和按对话导出都投影或读取这同一记录，不从聊天历史、提示词文本或再次检索中重建。
_Avoid_: 普通应用日志、聊天历史消息、工具过渡消息、重新查询结果

**角色知识映射**:
一个应用角色与世界书角色身份之间的稳定对应关系；同一应用角色的所有对话在后续检索中共享这项映射，无法可靠自动识别时由用户明确选择。修改映射不会清除既有对话已受原知识视角影响的内容，也不依赖显示名称持续推断。
_Avoid_: 角色名称匹配、逐对话角色选择、角色知识视角

**角色知识视角**:
角色知识映射在某轮世界书检索中解析出的角色身份，决定哪些 Character Thought 和有向 Character Relation State 可以作为该角色的主观知识参与检索。历史回复重新生成时使用该回合原先解析出的身份，不被之后的映射调整追溯改写。
_Avoid_: 当前角色名称、关系主体筛选参数、角色人设

**事件知情角色**:
依据剧情证据，可以从 Story Event 自身 `visible_from` 起安全获得该事件完整公开正文的角色。它通过 `known_by_character_ids` 显式正向授权，与事件参与者相互独立；Character Thought 链接既不是完整事件权限的充分条件，也不是必要条件。
_Avoid_: 事件参与者、Thought 持有角色、无链接即公开

**剧情进度**:
用户选择或接受界面明确显示的默认值后确定的作品内剧情上界；第一版分集作品只以“已看完第 1–13 集”等用户可理解的粒度表达，并对应到剧情时间轴上的当前时间点，不提供“尚未开始”或集内节点。首次选择季度根世界书包且尚未设置剧情进度时，界面明确选择并显示第 13 集；需要表达后作开始前的背景时选择前作根世界书及其第 13 集。
_Avoid_: 当前时间点、内部时间点、播放进度

**跨包状态续接**:
后作根世界书中的 Character Thought 或 Character Relation State 通过显式沿用依赖包的 Thought Thread Key 或 Relation Type Key，继续同一条主观认知或有向关系时间序列；运行时按依赖方向与新状态的开始时间解析当前有效状态，不能根据文本相似度自行认定续接。
_Avoid_: 跨包语义去重、根包任意覆盖、复制前作当前状态
