...
**桌宠形态**:
以原生透明 Qt 窗口呈现当前单角色对话的替代形态，与普通窗口共享对话、草稿和演出；不是独立会话或第二个播放渠道。
_Avoid_: Electron 桌宠、附加播放端

**共享对话运行时**:
拥有桌面单角色对话的提交、轮次、取消、分段合成与播放完成状态的公共业务层，不依赖聊天控件是否显示。
_Avoid_: ChatGUI、WebUI 后端

**单角色演出控制**:
普通 Pygame 窗口与 Qt 桌宠共用的音频队列、口型、动作及取消协调层；窗口和 OpenGL 上下文由各自宿主管理。
_Avoid_: Live2D adapter、窗口渲染器
...
**反馈提示词快照**:
提交反馈时采集的基础提示词与程序运行时指令，两段分别保存并可无损拼接。它不等同于被反馈的历史回复生成时实际使用的逐轮请求记录。
_Avoid_: 历史请求复现、完整请求日志

**反馈导入对话**:
由反馈创建、绑定到本机已加载角色的独立普通对话，使用基础提示词快照作为 Static 提示词，并保存本地来源信息。它不共用该角色的正常提示词，也不随云端反馈撤回或到期自动删除。
_Avoid_: 合并到当前对话、角色热加载、云端反馈缓存
