...
**默认 Live2D 模型**:
角色未指定对话级模型时用于展示的首选 Live2D 模型。没有默认模型的角色首次导入 Live2D 模型时，该模型成为默认模型；之后导入的其他模型属于可选模型。
_Avoid_: 当前对话模型、额外服装、上一次成功模型

**Live2D 模型目标**:
某次角色演出明确请求加载的默认模型、对话级模型或无模型状态。显式选择的对话级模型加载失败时，该目标是失败而不是自动改为默认模型。
_Avoid_: 上一次成功模型、静默 fallback 模型、角色展示能力

**Live2D 加载结果**:
一次 Live2D 模型目标加载得到的运行时结果，区分已加载、未配置和加载失败。它不改写用户的模型选择，也不是需要持久保存的角色属性。
_Avoid_: 模型配置、角色展示能力、持久错误状态

**空模型槽位**:
小剧场中保留角色名、字幕和播放节奏、但当前没有可绘制 Live2D 模型的角色槽位。它不等同于因 v2/v3 版本不一致而暂时隐藏的模型槽位。
_Avoid_: 版本冲突槽位、空角色、删除槽位
