...
对于用户可能自行修改、但发布版本需要强制恢复的文件，可在构建时重复指定 `--replace <相对路径>`。这类文件不会进入 hdiff，而是以完整内容放入补丁包的 `payload/` 目录，并在 manifest 中记录 `action: "replace"`、`payload`、`size` 和 `sha256`。应用端会先完成 hdiff，再校验并原子替换这些 payload；替换失败会与本次补丁的其他文件一起回滚。使用该动作的 manifest 应将 `min_updater_version` 设为 `1.1.0` 或更高。
