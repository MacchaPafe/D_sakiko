...
export function RuntimeIndicator({ connection, phase = 'idle', compact = false }) {
  ...
  const reconnecting = connection === 'connecting' || connection === 'checking_auth'
  const phaseText = {
    thinking: '思考中',
    tts: '生成语音',
    idle: '已连接',
  ...
  return (
    <span
      ...
      aria-label={online ? phaseText : (reconnecting ? '正在连接' : '连接中断')}
    >
      {online ? <Wifi size={14} /> : <WifiOff size={14} />}
      {!compact && (
        <span>{online ? phaseText : (reconnecting ? '正在连接' : '连接中断')}</span>
      )}
    </span>
  )
}
