    ...
    if os.path.exists('../API_Choice.json'):
        ...
        if active_provider is not None:
            ...
            if active_provider["name"]=="OpenAI":
                ...
                new_config_template["llm_setting"]["other_provider"][2]["if_choose"] = True
                new_config_template["llm_setting"]["other_provider"][2]["api_key"] = active_provider.get("api_key","")
                new_config_template["llm_setting"]["other_provider"][2]["model"] = active_provider.get("model","openrouter/auto")
            else:
...
class conf_ui(QWidget):
    def __init__(self):
        ...
        self.config["llm_setting"]["other_provider"] = [
            provider for provider in self.config["llm_setting"]["other_provider"]
            if provider.get("name") != "ModelScope"
        ]
        if not self.config["llm_setting"]["is_deepseek"]:
            has_choice = any(provider.get("if_choose") for provider in self.config["llm_setting"]["other_provider"])
            if not has_choice:
                self.config["llm_setting"]["is_deepseek"] = True
                if not self.config["llm_setting"].get("deepseek_key"):
    ...
    def initUI(self):
        ...
        self.radio5 = QRadioButton('OpenRouter')
    ...
    def set_api_conf_value(self):
        if self.llm_name in ['deepseek_up','deepseek_user']:
            self.api_llm_model_input.setText('deepseek V4')
    ...
    def radio_button_clicked(self,button):
        if button.text()=='使用up的deepseek':
            ...
            elif button.text()=='OpenRouter':
                self.llm_name='OpenRouter'
