...
class UserCharacterCard(QWidget):
    ...
    """
    def __init__(self, user_character: CharacterAttributes, parent=None):
        """
        创建一个用户信息显示卡片。
        """
    ...
    def load_character(self, character: CharacterAttributes | None = None) -> None:
        ...
        effective_icon_path = character.effective_icon_path
        if effective_icon_path:
            q_image = QImage(effective_icon_path)
        ...
        self.user_name_label.setText(character.effective_character_name)
...
class UserCharacterWidget(QWidget):
    ...
    def __init__(self, character: CharacterAttributes, radius=16, parent=None):
        """
        创建一个用户人设头像组件。
        """
        super().__init__(parent)
...
if __name__ == "__main__":
    ...
    widget = UserCharacterWidget(CharacterAttributes.create_user())
    widget.show()
    sys.exit(app.exec_())
