...
def validate_payload(payload: dict[str, Json]) -> None:
    ...
    if conversation is None:
        ...
        if payload["prompt_context"] is None or (isinstance(target, int) and target >= len(messages)):
...
def freeze_payload(*, app_version: str, rating: str, comment: str, chat: Chat | None = None,
                   system_prompt: str = "", target: int | None = None,
    ...
    if chat is not None:
        conversation = {"name": chat.name, "prompt_config": {"type": "static", "content": system_prompt},
                        "messages": [{"character_name": m.character_name, "text": m.text,
                                      "translation": m.translation, "emotion": m.emotion.as_string()}
                                     for m in chat.message_list]}
    payload: dict[str, Json] = {
        "schema_version": 1, "kind": "feedback", "request_id": request_id, "created_at": now,
        "app_version": app_version, "rating": rating, "comment": comment,
        "target": target, "conversation": conversation,
        "prompt_context": {"source": "rendered_at_feedback", "rendered_at": now} if chat else None,
    ...
    if len(body) > MAX_BODY:
        raise ValueError("整段对话和诊断超过 1 MiB，无法提交。可以从“意见建议”发送文字反馈；不会截断对话。")
    return request_id, body


def export_backup(payload: dict[str, Json], path: Path) -> None:
    """把反馈导出为仅含文本与 Static 提示词的现有备份格式。"""
    validate_payload(payload)
    conversation = payload["conversation"]
    ...
    chat: dict[str, Json] = {"chat_id": chat_id, "name": conversation["name"], "type": 1,
                            "prompt_config": conversation["prompt_config"], "start_message": "",
                            "meta": {}, "message_list": [{**m, "audio_path": "", "attachments": []} for m in messages]}
