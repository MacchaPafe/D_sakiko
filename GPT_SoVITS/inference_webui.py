...
path_sovits_v3 = ""
path_sovits_v4 = "pretrained_models/gsv-v4-pretrained/s2Gv4.pth"
...
def change_sovits_weights(sovits_path):
    ...
    vq_model.eval()
    if if_lora_v3 == False:
        print("加载SoVits模型", vq_model.load_state_dict(dict_s2["weight"], strict=False))
    else:
        path_sovits = path_sovits_v3 if model_version == "v3" else path_sovits_v4
        print(
            "加载预训练SoVITS模型: pretrained_G",
        ...
        print("加载sovits模型_lora秩：%s" % lora_rank)
...
def init_hifigan():
    ...
    print("加载vocoder", hifigan_model.load_state_dict(state_dict_g))
    if bigvgan_model:
...
def get_tts_wav(
    ...
    for i_text, text in enumerate(texts):
        ...
        t2 = ttime()
        message_queue.put(f"整理语言...\n{3*i_text+2}/{3*len(texts)+1}...")
        ...
        t4 = ttime()
        message_queue.put(f"整理语言...\n{3 * i_text + 4}/{3 * len(texts) + 1}...")
        t.extend([t2 - t1, t3 - t2, t4 - t3])
        t1= ttime()
    print(f"语音合成耗时{t[0]:.2f}+{sum(t[1::3]):.2f}+{sum(t[2::3]):.2f}+{sum(t[3::3]):.2f}={(t[0]+sum(t[1::3])+sum(t[2::3])+sum(t[3::3])):.2f}s")
