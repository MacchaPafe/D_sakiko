...
当前流水线支持按 `series_id` 选择系列标注配置。MyGO 的常用字段为：
...
梦限大使用 `series_id = SeriesId.YUME_MITA`；剧情学年无法确认时保持为空。时间线和可确认的剧情学年
不交给 LLM 判断，由任务元数据显式传入。`timeline_id` 用于隔离不能直接比较先后或共享有效区间的剧情
世界线，不是动画季度。
...
1. 系列/剧集先验角色表
- 先按 `series_id` 选择该动画的主要角色名单，不能静默回退到 MyGO
- 将来有需要时，可以在系列名单之上维护逐集的“高概率出场角色名单”
