...
class Live2DModel3NormalizationTestCase(unittest.TestCase):
    ...
    def test_normalize_model3_flattens_assets_and_builds_position_groups(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            text_generating_c = cast(list[dict[str, object]], motions["text_generating_C"])
            text_generating_l = cast(list[dict[str, object]], motions["text_generating_L"])
            text_generating_r = cast(list[dict[str, object]], motions["text_generating_R"])
            self.assertEqual(text_generating_c, text_generating_l)
            self.assertEqual(text_generating_l, text_generating_r)
