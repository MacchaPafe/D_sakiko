...
class DSLocalAndVoiceGen:
	def __init__(self,characters):
		...
		self.LLM_list=["deepseek-r1:14b","deepseek-r1:32b","deepseek-V4-Flash 非本地API","deepseek-V4-Pro 非本地API"]
		# if self.is_deepseek:
	...
	def text_generator(self,
		...
		while True:
			...
			while True:
				if user_input == 'm':
					if self.is_deepseek:
						message_queue.put("0：deepseek-r1:14b（需安装Ollama与对应本地大模型，选项1相同）  1：deepseek-r1:32b  \n2：调用deepseek-V4-Flash官方API（无需安装Ollama，只需联网)\n3：调用deepseek-V4-Pro官方API（支持思考模式，功能更强但消耗更多token）")
					else:
				...
				if user_wants_model.isdigit() and 0 <= int(user_wants_model) <= 1:
					...
					break
				elif user_wants_model.isdigit() and int(user_wants_model)==2:
					self.is_use_ds_api=True
					self.model_choice = self.LLM_list[2]
					break
				elif user_wants_model.isdigit() and int(user_wants_model)==3:
					self.is_use_ds_api=True
					self.model_choice = self.LLM_list[3]
					break
				else:
			...
			if not self.is_use_ds_api and self.is_deepseek:		#使用本地Ollama模型
				...
				if self.model_choice==self.LLM_list[2]:
					...
					online_model='deepseek-v4-pro'
				data = {
					"model": online_model,
					"messages": self.trim_list_to_340kb(to_llm_msg),
					"stream": False
				}
				if online_model == 'deepseek-v4-pro':
