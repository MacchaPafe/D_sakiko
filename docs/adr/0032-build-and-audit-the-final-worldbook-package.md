...
开发侧 Stage 3 审核工作台也以同一份 `worldbook_build.json` 作为正常入口，由它定位各集 Story/Lore Review、唯一的全量 Relation Review、Thought Review、Lore 去重决策及其直接证据，从而在一个包范围内展示完整审核进度和问题。单个审核 artifact 入口只作为受限的开发调试方式，不承担 Lore 跨文件对比、全包完成度或构建审计；审核工作台不复制构建配置中的输入清单，也不另建一份工作区配置。

审核工作台采用内存草稿与显式保存：字段、状态序列、备注和人工处置的界面修改在保存前不写入权威审核文件，切换条目或 artifact 时保留各自草稿，并持续显示逐文件未保存状态；普通保存与保存快捷键只对当前 artifact 执行完整 Schema 与业务校验，再通过安全替换写盘。工作台同时提供“保存全部”，先在内存中完成全部脏 artifact 的校验，再逐文件安全替换；不为多文件写入增加事务或回滚，罕见的部分写入失败必须逐文件报告并保留失败文件草稿供重试。重新加载或关闭仍有未保存修改的工作台必须提示，不按输入框变化自动保存，也不允许绕过完整 artifact 校验写出局部合法的 JSON。

保存直接上游后，工作台立即按构建配置和来源摘要把受影响的下游审核产物标记为来源过期，但不在保存动作中自动重生成。用户通过显式“重新生成受影响产物”操作查看将执行的既有流水线命令及预计迁移或重置范围，确认后才运行；完成后重新加载目标 artifact 并展示本次迁移报告。过期状态继续阻断构建审计，不能由界面静默刷新摘要或绕过。
...
四类审核产物都保留不可编辑的机器基准和可空的人工完整替换。Story Event 与 Lore Entry 使用 `generated_document` / `reviewed_document`，第一次人工修改时复制整条文档，之后只编辑人工文档；Relation Type 与 Thought Thread 使用 `generated_content` / `reviewed_content`，完整内容同时包含可编辑的主题字段和 State 序列。审核界面与发布器始终优先使用人工快照，人工快照不存在时才使用机器基准，正式包只保存最终投影，不保存双份数据或编辑标记。

四类候选都计算统一语义的 `review_basis_sha256`。Story Event 与 Lore Entry 摘要包含直接来源或证据稳定 ID 及完整规范化 `generated_document`；Relation Type 与 Thought Thread 摘要包含所覆盖局部证据稳定 ID、证据内容和完整规范化 `generated_content`。机器生成标题、文本、时间、标签、引用、分组或状态结构任一变化都会使旧审批失效；摘要排除人工快照、审核字段、备注、正式 UUID、文件路径和 JSON 排版。只有摘要完全相同时才迁移人工快照、审核处置与备注；摘要变化时旧人工内容只读展示供参考并重新审核，但正式身份仍可按身份继承规则保留。
...
审核工具提供显式“恢复机器版本”操作，删除对应 `reviewed_document` 或 `reviewed_content` 而不保留与机器基准相同的副本，同时把 `review_status` 重置为 `unreviewed`、清空最终 `disposition`，但保留非权威的 `suggested_disposition` 供重新审核；恢复后必须再次由人工完成审核。
...
第一版不允许按置信度或风险等级自动写入 `completed + publish`。所有候选都必须经明确的人工处置；审核工作台提供对用户显式勾选项目的“批量审核通过”，确认窗口按内容类型显示数量，并逐个审核单位写入 `completed + publish`，但不得称为批量发布，也不允许把当前筛选结果或未明确勾选项目自动纳入。存在身份待确认、结构错误或 `needs_followup` 的项目不能批量通过；`reject`、`exclude`、未归属 Observation 和未归属 Thought Update 仍逐项填写原因。Story/Lore 的审核单位仍是单条候选，Relation/Thought 的审核单位仍是完整序列而不是内部 State；机械一致的 Lore 去重是唯一不要求额外人工批准派生结果的例外。正式 `publish-worldbook(s)` 不选择部分候选，只有全部审核单位到达终态后才把所有处置为 `publish` 的内容整体构建进包。

统一审核字段为：`review_status` 取 `unreviewed`、`needs_followup` 或 `completed`；`disposition` 取 `publish`、`reject`、`exclude` 或空值；`suggested_disposition` 保存 LLM 或确定性规则的非权威建议；`disposition_reason` 在最终拒绝或排除时必填，发布时可空。审核未完成时最终 `disposition` 必须为空，生成器和 LLM response Schema 不能写入 `completed` 或最终处置，只有审核工具可以设置；发布要求 `review_status == completed` 且最终处置非空，并且只投影 `disposition == publish` 的候选。Relation 与 Thought 是否经过人工编辑由可空 `reviewed_content` 自动推导，不另存 `human_edited`；机械一致的 Lore 合并可以从全部已审核来源派生最终结果，无需伪造一次额外人工审核。
...
Relation Review 保存不可编辑的机器完整内容和可空的人工完整内容；发布器优先使用人工内容、为空时使用机器内容，并把其中的最终 State 投影为正式内容。正式包不保存双份内容或人工编辑标记。

Relation 的人工完整替换还必须覆盖可编辑的 `semantic_label`，Thought 的人工完整替换还必须覆盖可编辑的 `canonical_subject` 与 `thought_aspect`，不能让这三个参与最终语义投影的字段绕过 copy-on-write、审批重置和恢复机器版本规则。Relation 的主客体、Thought 的角色，以及两类记录的 series、timeline、branch 和开发侧稳定身份保持只读；这些归属错误时应拒绝候选或重新聚合，不在审核器中把候选改挂到另一个角色、角色对或世界范围。CLI 与审核工作台共同调用同一组类型化审核操作，CLI 的 replacement JSON 只是该接口的文件适配，不另有一套修改语义。

审核工作台允许在同一有向角色对内调整 Relation Observation 归属、拆分或合并 Relation Type，并增删、拆合、排序其 State；也允许在同一角色内调整 Thought Update 归属、拆分或合并 Thought Thread，并增删、拆合、排序其 State。这些结构修改作为完整人工快照保存并撤销既有审批，每条 Observation 或 Update 在有效审核结构中必须恰好归入一个序列，或者进入未归属队列并逐项拒绝或排除；不允许跨角色或跨有向角色对移动。Type、Thread 或 State 拆分合并必须显式处理开发侧身份继承，不能仅因内容相似而静默复用身份。Story/Lore 审核器不凭空新增完全缺少机器候选的知识，漏标应回到上游修正并重新生成。

Relation 的审核状态与最终处置放在同一有向角色对下的完整 Relation Type 上，不逐 State 保存；Type 处置为发布时，其审核后的所有 State 进入正式包，任何过度拆分、合并、重叠或时间边界问题都必须在完整内容内修正后再完成审核。Type 的拒绝表示聚合或分组错误，排除表示局部 Observation 有依据但不应形成长期关系状态。未合并 Observation 只能单独完成人工拒绝或排除，若要发布必须先纳入某个 Relation Type；发布前每条 Observation 必须恰好被一个已完成审核的 Type 覆盖，或自身已完成人工拒绝或排除。
...
Thought Review 保存不可编辑的机器完整内容和可空的人工完整内容；发布器优先使用人工内容、为空时使用机器内容，并把其中的最终 State 投影为正式内容。正式包不保存双份内容或人工编辑标记。

Thought 的审核状态与最终处置只放在完整 Thread 上，不逐 State 保存；Thread 处置为发布时，其审核后的整个 State 序列进入正式包，任何主题或 State 问题都必须在线程内修改、合并或删除后再完成审核，是否人工编辑由 `reviewed_content` 是否存在自动推导。Thread 的拒绝表示抽取或分组错误，排除表示内容有依据但不属于长期 Character Thought。尚未归入 Thread 的 Update 单独审核，只能最终拒绝或排除；若要发布，必须先归入或新建 Thread，`unresolved` Update 不能直接发布。发布前每个 Update 必须恰好由一个已完成审核的 Thread 覆盖，或自身已完成人工拒绝或排除。
...
目标 UUID 已不存在的孤立 Override 继续保存在用户状态中，不应用、不索引，也不降低同步 readiness；默认“全部条目”不显示它，但“我的内容”允许只读查看、导出和在直接选择所属包后永久删除，并以非阻塞徽标提示待处理数量。目标 UUID 恢复后旧 Override 继续按基准冲突和不兼容隔离规则重新适用。孤立 tombstone 仍保留但不显示、不自动删除，整包缺失的用户状态也不在运行时查看器中提供全局管理入口。
