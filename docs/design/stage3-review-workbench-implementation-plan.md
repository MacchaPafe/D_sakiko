...
> 2026-07-30 追加：Story Event 表单新增 `known_by_character_ids` 人工审核字段、“从参与角色复制”一次性操作，以及所有 effective Thought 反向链接的审核、处置与 stale 提示。提示不自动修改权限，也不阻断保存或完成审核。
>
> 2026-07-31 追加：来源过期可以通过版本化消费投影自动验证，或由审核者显式接受当前来源并保留审核结果；详见 ADR-0047。该操作不调用 LLM，不绕过缺失来源、Schema、范围或悬空引用错误。
...
- 显式重生成来源已过期的下游产物，或在确认变化不影响下游时重新确认来源。
...
5. 用户显式触发重生成，或通过消费投影比较后接受当前来源；两种操作都展示输入与预计影响。
...
来源重新确认同样由 GUI 与 CLI 共享的无界面模块实现。系统先计算版本化消费投影；投影一致时允许自动验证，投影不同、缺失旧基线或版本不兼容时要求人工说明。确认后立即安全保存新的 `direct_sources`，保留现有审核结果，并把当前完整投影基线及接受历史写入不纳入 Git 的 `*.source-acceptances.json`。来源缺失、无法解析、范围错误和悬空引用不能由人工确认绕过。
...
6. 接入来源消费投影预览、自动验证、人工接受、CLI dry-run 和本地 sidecar。

验收：从上游变化到重新生成或来源重新确认、重新审核、纯审计成功形成完整 GUI 闭环；正式发布仍只能从 CLI 进行。
