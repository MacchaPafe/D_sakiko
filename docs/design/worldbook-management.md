...
默认展示合并后真正会进入索引的有效条目，而不是把官方、Override、用户扩展和 tombstone 四层并列暴露。页面顶部使用 ComboBox 选择根世界书；下方左侧集中放置条目类型、添加入口、搜索、来源筛选和条目列表，右侧展示当前有效条目的类型化表单。来源筛选缩减为“全部条目、官方条目、我的内容”：“我的内容”包含普通 Override、孤立 Override 和用户扩展，单纯隐藏但没有 Override 的官方条目仍属于“官方条目”；“显示隐藏内容”独立控制带 tombstone 的官方身份。默认“全部条目”不展示孤立 Override，只有“我的内容”展示它们。筛选状态只在当前会话保留。当前根世界书依赖闭包中存在孤立 Override 时，筛选区旁显示“n 条修改需处理”的紧凑警告徽标；数量覆盖闭包内全部条目类型，没有孤立 Override 时隐藏徽标。点击徽标后切换到“我的内容”、切换到按稳定顺序排列的第一条孤立 Override 所属类型并直接打开该条目；导航顺序优先当前直接选中根包中可以立即处理的孤立 Override，再展示依赖包中的只读项，同一所有权层级内按条目类型、现有业务排序和 UUID 保持确定性。处理后立即刷新数量，再次点击可以定位下一条。孤立 Override 不参与有效内容和索引，因此该提示不把包或世界书索引降级。底部只在同步进行或失败时展示面向用户的状态，并把普通对账和破坏性全量重建明确区分。

Override 默认作为同一条目的修改后内容展示，使用“该条目已修改”提示，并允许切换查看官方内容；用户扩展显示“我添加的”，孤立 Override 显示醒目的“官方条目已不存在”标签，使“我的内容”中的三类用户内容仍可区分。孤立 Override 在目标官方条目不存在期间完全只读：能够由当前 adapter 解释时使用类型化表单展示用户保存的内容，并始终允许导出原始修改；不提供保存、恢复官方内容、保留修改或其他依赖官方基础的操作。只有孤立 Override 所属包被直接选为根世界书时才允许永久删除，依赖闭包中的孤立 Override 只读展示并提示切换到所属世界书，避免在浏览根包时误删依赖包用户状态。已隐藏官方条目默认不出现在有效列表，开启筛选后灰显并提供恢复。用户扩展不提供官方/修改后切换；只有扩展所属包被直接选为根世界书时才允许编辑和永久删除，依赖闭包中的扩展只读显示并提示切换所属世界书。不兼容扩展仍保留导航位置和当前根包下的永久删除入口，不提供绕过 Schema 的 JSON 修复编辑。

详情区“更多”菜单按当前持久条目状态只展示确实适用的条目操作，不用禁用项罗列其他状态的能力。正常官方条目只提供隐藏；附着普通 Override 的官方条目仍可隐藏，并同时提供恢复官方内容和导出原始修改，隐藏时保留 Override，恢复显示后继续使用用户版本；发生 Override 基准冲突时再提供保留修改和“打开用户状态文件位置”，后者在系统文件管理器中选中所属世界书的包级用户状态文件，而非打开 JSON 或定位单条记录。不兼容 Override 只提供恢复官方内容和导出原始修改，不提供隐藏、保留修改或用户状态文件入口。孤立 Override 始终提供导出原始修改；只有所属包直接作为根世界书时才额外提供永久删除孤立修改，依赖闭包中的孤立 Override 不提供删除或用户状态文件入口。用户扩展只在所属包直接作为根世界书时提供永久删除；不兼容用户扩展遵循相同所有权限制，依赖包中的不兼容扩展只提示切换所属世界书而不提供条目操作。适用动作沿用“隐藏条目、恢复官方内容、保留我的修改、导出原始修改、打开用户状态文件位置、永久删除条目”的固定相对顺序，不在条目操作内部增加分组。搜索对账和破坏性全量重建作为固定的世界书级维护组保留在菜单底部，只有同时存在条目操作时才在两组之间显示分隔线。状态不适用的动作直接隐藏；仅因任务正在运行而短暂不可执行的可见动作允许暂时禁用。隐藏、恢复官方内容、保留修改和永久删除等会刷新或改变当前条目状态的操作统一先处理未保存表单内容；导出和打开文件位置不触发未保存提示。

自由文本标签复用标签外观并支持显式可编辑模式，稳定 ID 多选项继续只显示和移除，不允许原位改写展示文字。自由标签可通过双击正文或悬停编辑按钮进入原位编辑；Enter 或失焦提交，Esc 取消，提交时去除首尾空白。空文本取消修改；改为已有标签时删除旧标签并复用已有项；普通改名保持原位置。原位编辑允许中英文逗号作为普通标签文字；底部新增框仍把逗号解释为批量添加多个标签，这是两个入口有意保留的交互差异。只读表单不提供任何编辑入口。

参与角色选择使用“角色乐队”组织当前九支主要乐队，默认顺序为 Poppin'Party、Afterglow、Hello, Happy World!、Pastel＊Palettes、Roselia、Morfonica、RAISE A SUILEN、MyGO!!!!!、Ave Mujica；不纳入 CRYCHIC 等历史乐队，也不抽象为团体或组合。乐队身份和成员关系属于可复用角色领域数据，选择对话框中的亲和排序属于 UI 策略，不改变世界书条目的持久字段。对话框使用不可选择的乐队标题分组；打开时把包含已选角色的乐队整体置顶，多支乐队命中时按各乐队首位已选角色在当前 `participants` 中的出现顺序排列。乐队内部保持固定成员顺序，其他乐队保持默认顺序；本次打开期间勾选变化不触发列表重排，下次打开时再按最新选择计算。显示排序与字段值顺序分离：未取消的原有选择保持相对顺序，新勾选角色按当前显示顺序追加，单纯打开并确认对话框不得改写 `participants` 或制造 Override。

参与角色搜索同时匹配角色中文名、稳定罗马音和乐队名。角色名或罗马音命中时只显示对应角色及其乐队标题；乐队名命中时显示该乐队标题和全部五名成员；没有可见成员的乐队标题隐藏。过滤不改变勾选状态，清空搜索后恢复本次打开时已经确定的分组顺序，不因本次勾选变化重新计算置顶乐队。

孤立 tombstone 不是用户写作内容，不纳入“我的内容”或孤立修改徽标；本轮继续保留但不展示、不自动删除，目标 UUID 恢复时重新应用隐藏状态。tombstone 只有 UUID 和基准 revision，缺少可放回按类型条目列表的正文与类型元数据，其高级管理不属于本轮范围。

整个所属官方世界书包未安装、manifest 无法读取或包无法加载时，对应用户状态文件继续保留，但不进入“我的内容”或孤立修改徽标，也不提供脱离所属包的全局删除入口。包重新安装或修复成功后再按包内规则识别孤立 Override；没有显示名称、时间线和类型上下文时不允许批量清理用户状态。

孤立 Override 的目标 UUID 重新出现在官方包中时，不增加等待用户重新激活的新状态：基准 revision 一致且内容兼容时正常恢复应用，基准不同但兼容时继续以用户版本为有效内容并提示复核，内容不兼容时隔离且不静默回退官方版本。用户已在孤立期间永久删除 Override 时只使用重新出现的官方内容。

永久删除孤立 Override 只原子更新所属包的用户状态，并从当前列表移除记录、刷新孤立修改数量、优先选择下一条孤立 Override；因为它原本不属于有效内容和索引，所以不启动索引同步，也不显示“正在更新搜索内容”。删除完成后只提示“修改已永久删除”。

类型 ComboBox 旁使用紧凑添加图标进入页内草稿。草稿复用官方条目的类型化字段，并增加不可折叠的“基础范围”卡片：Story Event 编辑系列和剧情分支，Character Thought 编辑角色、系列和剧情分支，Character Relation 编辑主体、目标、系列和剧情分支，Lore Entry 编辑剧情分支；时间线固定来自当前根包 manifest。创建与编辑都经过同一 Type Module 校验，检索文本留空时分别由事件摘要、观点正文、关系摘要与说话方式、Lore 标题与正文生成透明基础文本。切换条目、类型、世界书或关闭窗口时统一提示保存、放弃或取消。

用户扩展永久删除后保持当前来源筛选，并优先选择下一条、再选择上一条。剧情事件删除前使用包含隐藏内容和其他已安装包的全局持久条目目录查找 Character Thought 反向引用，并先展示完整影响预览。用户确认后，对能够安全写回的 Character Thought 从 `story_event_entry_ids` 中移除目标事件 UUID 并保留想法正文，不要求用户逐条处理，也不默认删除整条 Character Thought；无法安全解除的引用是否继续阻断删除由后续异常状态规则决定。官方条目仍只能隐藏，Override 仍只能删除替换后恢复官方版本。

级联解除引用写回已有 Override 时保留原 `base_revision`，不能把自动清理引用解释为用户已经复核当前官方基础；后续官方更新仍按完整替换和基准冲突规则处理。任何会让官方 Character Thought 重新成为有效内容的操作，包括删除 Override 恢复官方内容和恢复已隐藏官方条目，都必须先校验当前官方版本的 Story Event 引用仍然完整；引用目标不存在时阻止恢复并说明原因。

孤立 Character Thought Override 继续遵守完全只读规则，不因级联解除引用而原位改写。它引用待删除的 Story Event Extension 时，影响预览必须单独列出“将永久删除的孤立修改”，用户确认后删除整份孤立 Override；能够安全写回的普通 Character Thought 仍只移除事件引用并保留正文。

不兼容 Character Thought 不能假定移除一个 UUID 后即可安全写回：不兼容用户扩展在影响预览中列为“将永久删除的自定义条目”并整体删除；不兼容 Override 删除整份替换，目标官方 Character Thought 仍存在时先验证恢复后的官方引用完整；同时孤立且不兼容的 Override 按孤立 Override 整体删除。若删除 Override 会暴露一个引用仍不完整的官方版本，则整个级联操作阻断，不执行部分写入。

级联解除引用只自动修改当前直接选中所属包的一份用户状态。全局扫描发现其他包中的 Character Thought 引用时阻止删除，列出引用条目和所属包，并提供切换到该世界书的入口；不跨多个包顺序写入，不为这一异常状态引入多文件事务或回滚日志。同包引用约束补齐后，新的合法用户状态不再产生这种跨包阻断。

当前包中的纯官方 Character Thought 引用待删除事件时，级联操作不改写官方 JSON，而是以当前官方 revision 为基准创建一份完整 Override，仅移除目标事件 UUID 并保留其余内容；影响预览明确说明“将为官方角色想法保存修改”。完成后该条目归入“我的内容”，后续官方更新、恢复官方内容和引用校验均沿用普通 Override 规则。

删除影响预览逐条列出全部受影响 Character Thought，并按“移除关联并保留正文、为官方条目创建 Override、永久删除孤立修改、永久删除不兼容用户扩展、删除不兼容 Override 后恢复官方内容、位于其他包而阻断删除”分组；每项显示标题、角色、所属世界书和状态标签，数量较多时使用可滚动区域但不省略具体条目。没有阻断项时只提供“删除事件并应用以上更改”和“取消”，不存在绕过引用处理的强制删除；存在阻断项时不提供删除动作，只提供对应世界书的切换入口。

确认级联删除前先在内存中构造并校验当前包的完整目标用户状态；目标 Story Event Extension、需要更新或新增的 Override、需要更新或删除的用户扩展以及需要删除的孤立 Override 通过同一次 `WorldbookUserStateRepository.save()` 原子写入。任何预检、规范化或写入失败都保留原状态，不执行部分变更；成功后再启动一次索引同步，索引失败不回滚已经保存的用户状态。

实现中必须把“来源”和“状态”分开建模：普通 Override、孤立 Override 和用户扩展都映射到“我的内容”，孤立只是 Override 的派生状态，不能伪装成新的持久化层。导航记录必须直接携带原始孤立 Override、所属包和问题状态，不能依赖已经合并的有效条目反推出用户状态。供删除规划使用的全局持久目录必须分别保留官方条目、所有 Override（包括孤立 Override）、所有用户扩展和隐藏标记，不能先以 Override 替换官方记录后再扫描。

剧情事件删除实现分为纯规划与单次应用两步：规划器输入最新全部包、当前包用户状态和目标 Extension，输出按动作分组的受影响记录、跨包阻断项及完整目标状态，不执行写入；应用器只接受没有阻断项且仍对应当前用户状态的计划，再执行一次原子保存。至少覆盖无引用删除、普通 Override 解除引用且保留基准、用户扩展解除引用、纯官方想法创建 Override、孤立 Override 整体删除、不兼容内容整体删除或安全恢复、跨包阻断、预检失败零写入、保存成功后单次索引同步及索引失败不回滚的逻辑测试。
