...
第一轮最初暂不承诺、其中以下能力现已由后续迭代完成：

- 四类完整编辑表单；
- 新建、编辑与永久删除用户扩展；
- 完整 Override 编辑；
- “我的内容”统一筛选、孤立 Override 导航/导出/所属包删除；
- 剧情事件扩展删除影响预览、同包 Character Thought 级联处理和跨包阻断。

四类当前 Type Module 已形成稳定字段模型并完成解锁；原始 JSON 预览仍不属于第一版范围。后续 Schema 演进仍必须先补 adapter、类型校验和对应测试，不能回退到原始 JSON 编辑。
